...
- Analysis Pack：通过版本化快照导入属性、受治理指标与可信 Schema 策略；事件语义映射和 Dashboard 分别使用独立声明式接口。
- 项目 Dashboard：内置组件可按项目编排并引用受治理指标；只有内置组件无法表达的专属交互才需要可信 build-time extension（构建期扩展）。
