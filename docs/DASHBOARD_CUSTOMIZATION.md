...
Dashboard definition 的 `defaultRange` 定义首次进入该项目时的默认数据范围，支持 `24h`、`7d`、`30d`、`90d` 和 `custom`；`custom` 表示不预置日期，由用户自行选择。页面临时筛选不会静默改写这个项目级默认值；切换项目时按新项目的 Dashboard 重新初始化范围。
