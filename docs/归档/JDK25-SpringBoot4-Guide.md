---
title: JDK 25 与 Spring Boot 4 迁移踩坑记录
type: archive
status: archived
audience: human
scope: JDK 25 / Spring Boot 4 早期迁移排障经验
agent_notes: 不作为默认上下文；只有用户明确询问历史迁移或 JDK 25 踩坑时再阅读
---

# JDK 25 + Spring Boot 4 迁移指南与问题总结

> 归档说明：本文是人工参考材料，用于回看 JDK 25 / Spring Boot 4 早期踩坑和排障经验；不代表当前项目规约、部署指南或 Agent 默认上下文。当前开发、部署和运维口径以仓库根目录 `AGENTS.md`、`README.md`、`docs/README.md`、`ops/README.md` 和当前代码为准。

> 说明：本文是 JDK 25 + Spring Boot 4 升级过程中的迁移记录，主要用于解释技术选型和排障经验。
...
采用 JDK 25 + Spring Boot 4 构建 AnalyticsHub 后端服务：
- **JDK 25.0.1**
