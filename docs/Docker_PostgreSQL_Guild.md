...
# 创建用户与数据库（如已存在可忽略错误）
docker exec -e PGPASSWORD="$PGPASSWORD" "$PG_CONTAINER" psql -U "$PGUSER" -d postgres -c "CREATE ROLE demo_project LOGIN PASSWORD 'demo_project';"
docker exec -e PGPASSWORD="$PGPASSWORD" "$PG_CONTAINER" psql -U "$PGUSER" -d postgres -c "CREATE DATABASE demo_project OWNER demo_project;"
```
...
PG_CONTAINER=infra-postgres15
PGPASSWORD=your_db_password
docker exec -e PGPASSWORD="$PGPASSWORD" "$PG_CONTAINER" psql -U demo_project -d demo_project -c "SELECT 1;"
```
