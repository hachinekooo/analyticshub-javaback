...
public class AdminAuthenticationFilter extends OncePerRequestFilter {
    ...
    public AdminAuthenticationFilter(Environment environment, ObjectMapper objectMapper) {
    ...
    protected void doFilterInternal(
        ...
        if (!isManagementPath(request.getRequestURI()) || !isProdProfile()) {
            // Only protect actuator in production to keep local dev friction low.
            filterChain.doFilter(request, response);
            return;
        }

        if (hasQueryToken(request)) {
            sendErrorResponse(response, HttpServletResponse.SC_UNAUTHORIZED, "ADMIN_TOKEN_INVALID", "请使用请求头传递管理端 Token");
            return;
        }

        String token = extractAdminToken(request);
        if (token == null || token.isBlank()) {
            sendErrorResponse(response, HttpServletResponse.SC_UNAUTHORIZED, "ADMIN_TOKEN_MISSING", "缺少管理端 Token");
            return;
        }

        if (adminToken == null || adminToken.isBlank()) {
            sendErrorResponse(response, HttpServletResponse.SC_SERVICE_UNAVAILABLE, "ADMIN_TOKEN_NOT_CONFIGURED", "管理端 Token 未配置");
            return;
        }
